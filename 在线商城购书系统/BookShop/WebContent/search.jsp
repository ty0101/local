				...
				<div class="goodsbar">
					...
					<li style="float: left;">
						<c:if test="${searchBooks.currentPage<=1}">
							<a href="BookServlet?opt=search&page=${searchBooks.currentPage}">
							上一页
							</a>
						</c:if>
						<c:if test="${searchBooks.currentPage>1}">
							<a href="BookServlet?opt=search&page=${searchBooks.currentPage-1}">
							上一页
							</a>
						</c:if>
					</li>
					<li style="float: left;">
						...
						<c:if test="${searchBooks.currentPage>=searchBooks.totalPage}">
							<a href="BookServlet?opt=search&page=${searchBooks.totalPage}">
							下一页
							</a>
						</c:if>
					</li>
					...
					<div class="goodsbar-content">
						...
						<c:if test="${searchBooks !=null}" >
							<c:forEach items="${searchBooks.dataList}" var="book" >
								<dl class="bookList">
									<dt><a href="${ctxPath }/BookServlet?opt=buyBook&bid=${book.id}"><img src="images/book.jpg" class="" /></a></dt>
