		...
		<div id="container" >
			<c:if test="${user==null }">
				<div class="alert alert-warning alert-dismissible" role="alert" style="color:#FF6633;">
					...
					您还没有登录！登录后即可继续操作&nbsp;&nbsp;
					<a href="login.jsp" class="btn btn-danger btn-sm" style="height:25px;line-height: 13px;">立即登录</a>
				</div>
			</c:if>
			<!-- 购物车列表 -->
			<table class="table table-hover">
				...
				<c:forEach items="${silist}" var="s">
					<tr>
						<td><a href="BookServlet?opt=buyBook&bid=${s.item.id}">${s.item.title }</a></td>
						<td>￥${s.item.unitPrice }</td>
						<td>
						<div class="input-group input-group-sm">
						<a href="CartServlet?opt=deAmount&isbn=${s.item.isbn}" class="input-group-addon"  style="text-decoration: none;">-</a>
						<input id="num" type="text" value="${s.amount }" class="form-control input-sm text-center" onblur="uSAmount(this.value,${s.item.isbn})"/>
						<a href="CartServlet?opt=inAmount&isbn=${s.item.isbn}" class="input-group-addon" style="text-decoration: none;">+</a>
						</div>
						</td>
			...
			<c:if test="${cart!=null && cart.itemAmount>0 }">
				<div class="pull-right">
					...
					<a href="login.jsp" class="btn btn-danger  btn-sm">去结算</a>
					<a href="CartServlet?opt=clear" class="btn btn-warning btn-sm">清空购物车</a>
				</div>
